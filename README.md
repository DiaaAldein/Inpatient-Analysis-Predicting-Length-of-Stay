...
| **A large train/test leakage problem, not a minor one** - dropping non-predictive columns left many genuinely different patients identical across every remaining column including the target; a naive random split was measured to place the same combination on both sides for an estimated **70.8% of the test set** | Kept every row (the duplication reflects real outcome variance, not a data error) but replaced random splitting with a group-aware, stratified split (`StratifiedGroupKFold`), grouping rows by a hash of their full feature+target combination so identical combinations can never be separated across the split - verified afterward at zero cross-contaminated rows. See [Section 5](docs/METHODOLOGY.md#section-5-feature-selection--traintest-split). |
